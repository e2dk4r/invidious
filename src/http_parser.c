...
internalfn void
HttpParserMustBeJson(struct http_parser *parser, struct json_token *tokens, u32 tokenMax, u32 *tokenCount)
{
  ...
  parser->jsonParser = JsonParser(tokens, tokenMax);
}
...
HttpParserParse(struct http_parser *parser, struct string *httpResponse)
{
  ...
  while (1) {
    ...
    u16 chunkSize;
    if (!ParseHex(&chunkSizeText, (u64 *)&chunkSize)) {
    ...
    struct string chunkData = StringCursorConsumeUntilCRLF(&cursor);
    ...
    // chunkData
    u32 breakHere = 1;
    struct json_parser *jsonParser = &parser->jsonParser;
